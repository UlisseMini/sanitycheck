...
describe('formatForExtension()', () => {
  ...
  it('passes through valid JSON directly without calling API', async () => {
    ...
    const result = await formatForExtension(validJson, 'original article text');
  ...
  it('calls Haiku with tool use for free-form text', async () => {
    ...
    const analysisText = 'The article argues that X is true. The main issue is that the author quotes "Test quote from the article" but this assumes Y without justification.';
    const originalText = 'Original article text here. Test quote from the article. More text.';
    ...
    if (!USE_REAL_API) {
      ...
      expect(['none', 'minor', 'moderate', 'significant']).toContain(result.severity);
    }
  });
  ...
  it('handles analysis with no issues', async () => {
    ...
    const analysisText = 'The article claims that regular exercise improves health. No significant logical issues found.';
    const originalText = 'Regular exercise has been shown to improve overall health outcomes.';
    const result = await formatForExtension(analysisText, originalText);
  ...
  it.skipIf(USE_REAL_API)('throws if no tool use in response', async () => {
    ...
    await expect(formatForExtension('Some analysis', 'Original text'))
